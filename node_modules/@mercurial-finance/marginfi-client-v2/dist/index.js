...
var MarginfiConfigRaw = _superstruct.object.call(void 0, {
  label: _superstruct.enums.call(void 0, ["production", "alpha", "staging", "dev", "mainnet-test-1", "dev.1"]),
...
var _mrgncommon = require('@mrgnlabs/mrgn-common');
...
async function makeRepayIx(mfProgram, accounts, args, remainingAccounts = []) {
  return mfProgram.methods.lendingAccountRepay(args.amount, _nullishCoalesce(args.repayAll, () => ( null))).accounts({
    marginfiGroup: accounts.marginfiGroupPk,
    marginfiAccount: accounts.marginfiAccountPk,
    signer: accounts.authorityPk,
    signerTokenAccount: accounts.signerTokenAccountPk,
...
async function makeWithdrawIx(mfProgram, accounts, args, remainingAccounts = []) {
  return mfProgram.methods.lendingAccountWithdraw(args.amount, _nullishCoalesce(args.withdrawAll, () => ( null))).accounts({
    marginfiGroup: accounts.marginfiGroupPk,
    marginfiAccount: accounts.marginfiAccountPk,
...
var MarginfiAccount = class {
  ...
  async makeDepositIx(amount, bank) {
    ...
    const ix = await instructions_default.makeDepositIx(
      ...
      { amount: _mrgncommon.uiToNative.call(void 0, amount, bank.mintDecimals) },
      remainingAccounts
    );
    return {
      instructions: bank.mint.equals(_mrgncommon.NATIVE_MINT) ? await this.wrapInstructionForWSol(ix, amount) : [ix],
      keys: []
    };
  }
  /**
  ...
  async makeRepayIx(amount, bank, repayAll = false) {
    ...
    const ix = await instructions_default.makeRepayIx(
      ...
      { amount: _mrgncommon.uiToNative.call(void 0, amount, bank.mintDecimals), repayAll },
      remainingAccounts
    );
    return {
      instructions: bank.mint.equals(_mrgncommon.NATIVE_MINT) ? await this.wrapInstructionForWSol(ix, amount) : [ix],
  ...
  async repay(amount, bank, repayAll = false) {
    ...
    if (repayAll && !bank.emissionsMint.equals(_web3js.PublicKey.default)) {
      const userAta = await _token.associatedAddress.call(void 0, {
        mint: bank.emissionsMint,
        owner: this.client.provider.wallet.publicKey
      });
      const createAtaIdempotentIx = _mrgncommon.createAssociatedTokenAccountIdempotentInstruction.call(void 0, 
  ...
  async makeWithdrawIx(amount, bank, withdrawAll = false) {
    ...
    const ix = await instructions_default.makeWithdrawIx(
      ...
      { amount: _mrgncommon.uiToNative.call(void 0, amount, bank.mintDecimals), withdrawAll },
      remainingAccounts
    );
    return { instructions: bank.mint.equals(_mrgncommon.NATIVE_MINT) ? await this.wrapInstructionForWSol(ix) : [ix], keys: [] };
  }
  /**
  ...
  async withdraw(amount, bank, withdrawAll = false) {
    const tx = new (0, _web3js.Transaction)();
    if (withdrawAll && !bank.emissionsMint.equals(_web3js.PublicKey.default)) {
      const userAta2 = await _token.associatedAddress.call(void 0, {
        mint: bank.emissionsMint,
        owner: this.client.provider.wallet.publicKey
      });
      const createAtaIdempotentIx2 = _mrgncommon.createAssociatedTokenAccountIdempotentInstruction.call(void 0, 
        this.client.provider.wallet.publicKey,
        userAta2,
        this.client.provider.wallet.publicKey,
        bank.emissionsMint
  ...
  async makeBorrowIx(amount, bank, opt) {
    ...
    const remainingAccounts = this.getHealthCheckAccounts(
      (_nullishCoalesce(_optionalChain([opt, 'optionalAccess', _6 => _6.remainingAccountsBankOverride, 'optionalAccess', _7 => _7.length]), () => ( 0))) > 0 ? _optionalChain([opt, 'optionalAccess', _8 => _8.remainingAccountsBankOverride]) : [bank]
    );
  ...
  getHealthCheckAccounts(mandatoryBanks = [], excludedBanks = []) {
    ...
    return this._lendingBalances.filter((balance) => {
      ...
      return [
        {
          pubkey: new (0, _web3js.PublicKey)(bankPk),
          isSigner: false,
          isWritable: false
        },
        {
  ...
  static async _fetchAccountData(accountAddress, config, program, commitment) {
    const mergedCommitment = _nullishCoalesce(_nullishCoalesce(commitment, () => ( program.provider.connection.commitment)), () => ( _mrgncommon.DEFAULT_COMMITMENT));
  ...
  async reload() {
    ...
    const banks = await Promise.all(
      bankAccountsData.map(async (accountData) => {
        ...
        return new Bank(
          _optionalChain([this, 'access', _9 => _9._config, 'access', _10 => _10.banks, 'access', _11 => _11.find, 'call', _12 => _12((b) => b.address.equals(accountData.publicKey)), 'optionalAccess', _13 => _13.label]) || "Unknown",
  ...
  async _loadGroupAndAccountAi() {
    let [marginfiGroupAi, marginfiAccountAi] = await this.client.provider.connection.getMultipleAccountsInfo(
      ...
      _mrgncommon.DEFAULT_COMMITMENT
    );
  ...
  getHealthComponents(marginReqType) {
    const [assets, liabilities] = this.activeBalances.map((accountBalance) => {
      ...
      if (!bank)
        throw Error(`Bank ${_mrgncommon.shortenAddress.call(void 0, accountBalance.bankPk)} not found`);
  ...
  getBalance(bankPk) {
    return _nullishCoalesce(this.activeBalances.find((b) => b.bankPk.equals(bankPk)), () => ( Balance.newEmpty(bankPk)));
  }
  getFreeCollateral() {
  ...
  computeNetApy() {
    ...
    return _mrgncommon.aprToApy.call(void 0, apr);
  }
  /**
  ...
  async makeLendingAccountLiquidateIx(liquidateeMarginfiAccount, assetBank, assetQuantityUi, liabBank) {
    const ix = await instructions_default.makeLendingAccountLiquidateIx(
      ...
      { assetAmount: _mrgncommon.uiToNative.call(void 0, assetQuantityUi, assetBank.mintDecimals) },
      [
        {
  ...
  async lendingAccountLiquidate(liquidateeMarginfiAccount, assetBank, assetQuantityUi, liabBank) {
    ...
    const tx = new (0, _web3js.Transaction)().add(...ixw.instructions, _web3js.ComputeBudgetProgram.setComputeUnitLimit({ units: 6e5 }));
  ...
  getMaxLiquidatableAssetAmount(assetBank, liabBank) {
    ...
    const liquidationDiscount = new (0, _bignumberjs2.default)(1 - 0.05);
    ...
    return _bignumberjs2.default.min(assetBalanceBound, liabBalanceBound, maxLiquidatableUnboundedAssetAmount);
  }
  describe() {
    const { assets, liabilities } = this.getHealthComponents(2 /* Equity */);
    return `
...
- Balances:  ${this.activeBalances.map((la) => {
  ...
  async wrapInstructionForWSol(ix, amount = new (0, _bignumberjs2.default)(0)) {
    return [...await this.makeWrapSolIxs(new (0, _bignumberjs2.default)(amount)), ix, await this.makeUnwrapSolIx()];
  }
  async makeWrapSolIxs(amount) {
    const address = await _token.associatedAddress.call(void 0, { mint: _mrgncommon.NATIVE_MINT, owner: this.client.wallet.publicKey });
    const ixs = [
      _mrgncommon.createAssociatedTokenAccountIdempotentInstruction.call(void 0, 
        this.client.wallet.publicKey,
        address,
        this.client.wallet.publicKey,
        _mrgncommon.NATIVE_MINT
      )
    ];
    if (amount.gt(0)) {
      const nativeAmount = _mrgncommon.uiToNative.call(void 0, amount, 9).toNumber() + 1e4;
      ixs.push(
        _web3js.SystemProgram.transfer({ fromPubkey: this.client.wallet.publicKey, toPubkey: address, lamports: nativeAmount }),
        _mrgncommon.createSyncNativeInstruction.call(void 0, address)
      );
    }
    return ixs;
  }
  async makeUnwrapSolIx() {
    const address = await _token.associatedAddress.call(void 0, { mint: _mrgncommon.NATIVE_MINT, owner: this.client.wallet.publicKey });
    return _mrgncommon.createCloseAccountInstruction.call(void 0, address, this.client.wallet.publicKey, this.client.wallet.publicKey);
  }
};
...
var Balance = class {
  constructor(data) {
    ...
    this.assetShares = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, data.assetShares);
    this.liabilityShares = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, data.liabilityShares);
    this.emissionsOutstanding = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, data.emissionsOutstanding);
  ...
  static newEmpty(bankPk) {
    return new Balance({
      active: false,
      bankPk,
      assetShares: { value: new (0, _anchor.BN)(0) },
      liabilityShares: { value: new (0, _anchor.BN)(0) },
      emissionsOutstanding: { value: new (0, _anchor.BN)(0) },
      lastUpdate: 0
    });
  }
  ...
  getQuantityUi(bank) {
    return {
      assets: new (0, _bignumberjs2.default)(_mrgncommon.nativeToUi.call(void 0, bank.getAssetQuantity(this.assetShares), bank.mintDecimals)),
      liabilities: new (0, _bignumberjs2.default)(_mrgncommon.nativeToUi.call(void 0, bank.getLiabilityQuantity(this.liabilityShares), bank.mintDecimals))
    };
  }
  ...
  calcClaimedEmissions(bank, currentTimestamp) {
    ...
    if (balanceAmount) {
      ...
      const period = new (0, _bignumberjs2.default)(currentTimestamp - lastUpdate);
      const emissionsRate = new (0, _bignumberjs2.default)(bank.emissionsRate);
      const emissions = period.times(balanceAmount).times(emissionsRate).div(31536e9);
      const emissionsReal = _bignumberjs2.default.min(emissions, new (0, _bignumberjs2.default)(bank.emissionsRemaining));
...
var PYTH_PRICE_CONF_INTERVALS = new (0, _bignumberjs2.default)(2.12);
var SWB_PRICE_CONF_INTERVALS = new (0, _bignumberjs2.default)(2.12);
var USDC_DECIMALS = 6;

// src/bank.ts
var _client = require('@pythnetwork/client');

var _solanajs = require('@switchboard-xyz/solana.js');
var Bank = class {
  constructor(label, address, rawData, priceData) {
    ...
    this.assetShareValue = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.assetShareValue);
    this.liabilityShareValue = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.liabilityShareValue);
    ...
    this.collectedInsuranceFeesOutstanding = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.collectedInsuranceFeesOutstanding);
    ...
    this.collectedGroupFeesOutstanding = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.collectedGroupFeesOutstanding);
    this.config = {
      assetWeightInit: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.assetWeightInit),
      assetWeightMaint: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.assetWeightMaint),
      liabilityWeightInit: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.liabilityWeightInit),
      liabilityWeightMaint: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.liabilityWeightMaint),
      depositLimit: _mrgncommon.nativeToUi.call(void 0, rawData.config.depositLimit, this.mintDecimals),
      oracleSetup: rawData.config.oracleSetup,
      oracleKeys: rawData.config.oracleKeys,
      interestRateConfig: {
        insuranceFeeFixedApr: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.insuranceFeeFixedApr),
        maxInterestRate: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.maxInterestRate),
        insuranceIrFee: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.insuranceIrFee),
        optimalUtilizationRate: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.optimalUtilizationRate),
        plateauInterestRate: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.plateauInterestRate),
        protocolFixedFeeApr: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.protocolFixedFeeApr),
        protocolIrFee: _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.config.interestRateConfig.protocolIrFee)
      }
    };
    this.totalAssetShares = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.totalAssetShares);
    this.totalLiabilityShares = _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.totalLiabilityShares);
    ...
    this.emissionsRemaining = rawData.emissionsRemaining ? _mrgncommon.wrappedI80F48toBigNumber.call(void 0, rawData.emissionsRemaining) : new (0, _bignumberjs2.default)(0);
  }
  describe() {
    return `
...
Total deposits: ${_mrgncommon.nativeToUi.call(void 0, this.totalAssets, this.mintDecimals)}
Total borrows: ${_mrgncommon.nativeToUi.call(void 0, this.totalLiabilities, this.mintDecimals)}
...
LTVs:
- Initial: ${new (0, _bignumberjs2.default)(1).div(this.config.liabilityWeightInit).times(100).toFixed(2)}%
- Maintenance: ${new (0, _bignumberjs2.default)(1).div(this.config.liabilityWeightMaint).times(100).toFixed(2)}%
`;
  }
  get totalAssets() {
  ...
  getUsdValue(quantity, priceBias, weight, scaleToBase = true) {
    ...
    return quantity.times(price).times(_nullishCoalesce(weight, () => ( 1))).dividedBy(scaleToBase ? 10 ** this.mintDecimals : 1);
  }
  ...
  async getEmissionsData(connection) {
    const mint = await _mrgncommon.getMint.call(void 0, connection, this.emissionsMint);
    ...
    return {
      ...
      rateUi: new (0, _bignumberjs2.default)(rateUi),
      remainingUi
    };
  }
};
...
async function getOraclePriceData(connection, oracleSetup, oracleKeys) {
  switch (oracleSetup) {
    case 1 /* PythEma */:
      ...
      const pythPriceData = _client.parsePriceData.call(void 0, account.data);
      const pythPrice = new (0, _bignumberjs2.default)(pythPriceData.emaPrice.value);
      const pythConfInterval = new (0, _bignumberjs2.default)(pythPriceData.emaConfidence.value);
    ...
    case 2 /* SwitchboardV2 */:
      const swbProgram = await _solanajs.SwitchboardProgram.load("mainnet-beta", connection);
      const aggAccount = new (0, _solanajs.AggregatorAccount)(swbProgram, oracleKeys[0]);
      const aggData = await aggAccount.loadData();
      const swbPrice = new (0, _bignumberjs2.default)(_solanajs.AggregatorAccount.decodeLatestValue(aggData).toString());
      const swbConfidence = new (0, _bignumberjs2.default)(aggData.latestConfirmedRound.stdDeviation.toBig().toString());
...
var MarginfiGroup = class {
  ...
  static async fetch(config, program, commitment) {
    ...
    const banks = await Promise.all(
      bankAccountsData.map(async (accountData2) => {
        ...
        return new Bank(
          _optionalChain([config, 'access', _14 => _14.banks, 'access', _15 => _15.find, 'call', _16 => _16((b) => b.address.equals(accountData2.publicKey)), 'optionalAccess', _17 => _17.label]) || "Unknown",
  ...
  async reload(commitment) {
    ...
    const banks = await Promise.all(
      bankAccountsData.map(async (accountData) => {
        let bankData = accountData.account;
        return new Bank(
          _optionalChain([this, 'access', _18 => _18._config, 'access', _19 => _19.banks, 'access', _20 => _20.find, 'call', _21 => _21((b) => b.address.equals(accountData.publicKey)), 'optionalAccess', _22 => _22.label]) || "Unknown",
          accountData.publicKey,
          bankData,
          await getOraclePriceData(
            this._program.provider.connection,
  ...
  getBankByMint(mint) {
    return _nullishCoalesce([...this._banks.values()].find((bank) => bank.mint.equals(mint)), () => ( null));
  }
};
...
var MarginfiClient = class {
  ...
  static async fetch(config, wallet, connection, opts) {
    ...
    const program = new (0, _anchor.Program)(IDL, config.programId, provider);
    return new MarginfiClient(config, program, wallet, await group_default.fetch(config, program, _optionalChain([opts, 'optionalAccess', _23 => _23.commitment])));
  }
  static async fromEnv(overrides) {
    const env = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _24 => _24.env]), () => ( process.env.MARGINFI_ENV));
    const connection = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _25 => _25.connection]), () => ( new (0, _web3js.Connection)(process.env.MARGINFI_RPC_ENDPOINT, {
      commitment: _mrgncommon.DEFAULT_COMMITMENT
    })));
    const programId = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _26 => _26.programId]), () => ( new (0, _web3js.PublicKey)(process.env.MARGINFI_PROGRAM)));
    const groupPk = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _27 => _27.marginfiGroup]), () => ( (process.env.MARGINFI_GROUP ? new (0, _web3js.PublicKey)(process.env.MARGINFI_GROUP) : _web3js.PublicKey.default)));
    const wallet = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _28 => _28.wallet]), () => ( new (0, _mrgncommon.NodeWallet)(
      process.env.MARGINFI_WALLET_KEY ? _web3js.Keypair.fromSecretKey(new Uint8Array(JSON.parse(process.env.MARGINFI_WALLET_KEY))) : _mrgncommon.loadKeypair.call(void 0, process.env.MARGINFI_WALLET)
    )));
  ...
  async createMarginfiAccount(opts) {
    ...
    return _optionalChain([opts, 'optionalAccess', _29 => _29.dryRun]) ? Promise.resolve(void 0) : account_default.fetch(accountKeypair.publicKey, this, _optionalChain([opts, 'optionalAccess', _30 => _30.commitment]));
  }
  /**
    ...
    return (await this.program.provider.connection.getProgramAccounts(this.programId, {
      ...
      filters: [
        ...
        },
        {
          memcmp: {
            offset: 0,
            bytes: _bytes.bs58.encode(_anchor.BorshAccountsCoder.accountDiscriminator("marginfiAccount" /* MarginfiAccount */))
          }
        }
      ]
    })).map((a) => a.pubkey);
      ...
      filters: [
        {
          memcmp: {
            offset: 0,
            bytes: _bytes.bs58.encode(_anchor.BorshAccountsCoder.accountDiscriminator(type))
          }
        }
      ]
    })).map((a) => a.pubkey);
  ...
  async processTransaction(transaction, signers, opts) {
    ...
    try {
      ...
      const connection = new (0, _web3js.Connection)(this.provider.connection.rpcEndpoint, this.provider.opts);
      const {
        context: { slot: minContextSlot },
        value: { blockhash, lastValidBlockHeight }
      } = await connection.getLatestBlockhashAndContext();
      if (transaction instanceof _web3js.Transaction) {
        const versionedMessage = new (0, _web3js.TransactionMessage)({
          instructions: transaction.instructions,
          payerKey: this.provider.publicKey,
          recentBlockhash: blockhash
        });
        versionedTransaction = new (0, _web3js.VersionedTransaction)(versionedMessage.compileToV0Message([]));
      } else {
      ...
      if (_optionalChain([opts, 'optionalAccess', _31 => _31.dryRun])) {
        ...
        const signaturesEncoded = encodeURIComponent(
          JSON.stringify(versionedTransaction.signatures.map((s) => _bytes.bs58.encode(s)))
        );
        ...
        let mergedOpts = {
          ..._mrgncommon.DEFAULT_CONFIRM_OPTS,
          commitment: _nullishCoalesce(connection.commitment, () => ( _mrgncommon.DEFAULT_CONFIRM_OPTS.commitment)),
          preflightCommitment: _nullishCoalesce(connection.commitment, () => ( _mrgncommon.DEFAULT_CONFIRM_OPTS.commitment)),
          minContextSlot,
          ...opts
        };
      ...
      throw `Transaction failed! ${_optionalChain([error, 'optionalAccess', _32 => _32.message])}`;
    }
  }
};
  ...
  async reload() {
    ...
    const banks = await Promise.all(
      bankAccountsData.map(async (accountData) => {
        let bankData = accountData.account;
        return new Bank(
          _optionalChain([this, 'access', _33 => _33._config, 'access', _34 => _34.banks, 'access', _35 => _35.find, 'call', _36 => _36((b) => b.address.equals(accountData.publicKey)), 'optionalAccess', _37 => _37.label]) || "Unknown",
          accountData.publicKey,
          bankData,
          await getOraclePriceData(
            this._program.provider.connection,
...
var MarginfiClientReadonly = class {
  ...
  static async fetch(config, connection, opts) {
    ...
    const program = new (0, _anchor.Program)(IDL, config.programId, provider);
    return new MarginfiClientReadonly(config, program, await group_default.fetch(config, program, _optionalChain([opts, 'optionalAccess', _38 => _38.commitment])));
  }
  static async fromEnv(overrides) {
    const env = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _39 => _39.env]), () => ( process.env.MARGINFI_ENV));
    const connection = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _40 => _40.connection]), () => ( new (0, _web3js.Connection)(process.env.MARGINFI_RPC_ENDPOINT, {
      commitment: _mrgncommon.DEFAULT_COMMITMENT
    })));
    const programId = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _41 => _41.programId]), () => ( new (0, _web3js.PublicKey)(process.env.MARGINFI_PROGRAM)));
    const groupPk = _nullishCoalesce(_optionalChain([overrides, 'optionalAccess', _42 => _42.marginfiGroup]), () => ( (process.env.MARGINFI_GROUP ? new (0, _web3js.PublicKey)(process.env.MARGINFI_GROUP) : _web3js.PublicKey.default)));
    const config = await getConfig(env, {
      groupPk: _anchor.translateAddress.call(void 0, groupPk),
      programId: _anchor.translateAddress.call(void 0, programId)
    });
...
function getBankVaultAuthority(bankVaultType, bankPk, programId) {
  return _web3js.PublicKey.findProgramAddressSync([getBankVaultAuthoritySeeds(bankVaultType), bankPk.toBuffer()], programId);
}
...
exports.AccountType = AccountType; exports.Balance = Balance; exports.Bank = Bank; exports.BankVaultType = BankVaultType; exports.MARGINFI_IDL = IDL; exports.MarginRequirementType = MarginRequirementType; exports.MarginfiAccount = account_default; exports.MarginfiClient = client_default; exports.MarginfiClientReadonly = clientReadonly_default; exports.MarginfiGroup = group_default; exports.OracleSetup = OracleSetup; exports.PDA_BANK_FEE_VAULT_AUTH_SEED = PDA_BANK_FEE_VAULT_AUTH_SEED; exports.PDA_BANK_FEE_VAULT_SEED = PDA_BANK_FEE_VAULT_SEED; exports.PDA_BANK_INSURANCE_VAULT_AUTH_SEED = PDA_BANK_INSURANCE_VAULT_AUTH_SEED; exports.PDA_BANK_INSURANCE_VAULT_SEED = PDA_BANK_INSURANCE_VAULT_SEED; exports.PDA_BANK_LIQUIDITY_VAULT_AUTH_SEED = PDA_BANK_LIQUIDITY_VAULT_AUTH_SEED; exports.PDA_BANK_LIQUIDITY_VAULT_SEED = PDA_BANK_LIQUIDITY_VAULT_SEED; exports.PYTH_PRICE_CONF_INTERVALS = PYTH_PRICE_CONF_INTERVALS; exports.PriceBias = PriceBias; exports.SWB_PRICE_CONF_INTERVALS = SWB_PRICE_CONF_INTERVALS; exports.USDC_DECIMALS = USDC_DECIMALS; exports.getBankVaultAuthority = getBankVaultAuthority; exports.getBankVaultSeeds = getBankVaultSeeds; exports.getConfig = getConfig; exports.getOraclePriceData = getOraclePriceData;
