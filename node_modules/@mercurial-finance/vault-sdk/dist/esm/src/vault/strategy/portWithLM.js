...
export default class PortWithLMHandler {
    ...
    withdraw(walletPubKey, program, strategy, vault, tokenVault, vaultState, userToken, userLp, amount, preInstructions, postInstructions, opt) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let [collateralVault] = PublicKey.findProgramAddressSync([Buffer.from(SEEDS.COLLATERAL_VAULT_PREFIX), strategyBuffer], program.programId);
            const rewarder = new PublicKey(REWARDER);
            const [quarryPda] = PublicKey.findProgramAddressSync([Buffer.from(SEEDS.QUARRY), rewarder.toBuffer(), portCollateral.mintPubkey.toBuffer()], quarry.QUARRY_ADDRESSES.Mine);
            const [miner] = PublicKey.findProgramAddressSync([Buffer.from(SEEDS.MINER), quarryPda.toBuffer(), new PublicKey(vault).toBuffer()], quarry.QUARRY_ADDRESSES.Mine);
