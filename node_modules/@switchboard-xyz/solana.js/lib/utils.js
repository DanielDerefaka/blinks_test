...
export function handleOptionalPubkeys(ixn) {
 ...
 * ```
 */
export async function createStaticFeed(queueAccount, params, timeout = 30000) {
    const [aggregatorAccount] = await queueAccount.createFeed({
        ...
        jobs: [
            {
                weight: 1,
                data: OracleJob.encodeDelimited(OracleJob.fromObject({
                    tasks: [
                        {
                            valueTask: {
                                value: params.value,
 ...
 * ```
 */
export async function updateStaticFeed(aggregatorAccount, value, timeout = 30000) {
    const aggregator = await aggregatorAccount.loadData();
    const [jobAccount, jobInit] = JobAccount.createInstructions(aggregatorAccount.program, aggregatorAccount.program.walletPubkey, {
        keypair: Keypair.generate(),
        data: OracleJob.encodeDelimited(OracleJob.create({
            tasks: [
    ...
    const oldJobKeys = aggregator.jobPubkeysData.filter((pubkey) => !pubkey.equals(PublicKey.default));
...
export function parseRawBuffer(rawBuffer, size = 32) {
    let myUint8Array;
...
export function containsMrEnclave(mrEnclaves, targetMrEnclave) {
...
export function numToBN(num, defaultVal = 0) {
    if (!num) {
...
export function calculatePriorityFee(timestamp, roundOpenTimestamp, basePriorityFee, priorityFeeBump, priorityFeeBumpPeriod, maxPriorityFeeMultiplier) {
