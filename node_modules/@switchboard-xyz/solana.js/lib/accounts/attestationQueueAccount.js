 ...
 */
class AttestationQueueAccount extends Account {
    ...
    async createVerifierInstruction(payer, params, options) {
        ...
        const [verifierAccount, quoteInit] = await VerifierAccount.createInstruction(this.program, payer, { ...params, queueAccount: this, authority }, options);
        if (!params.createPermissions && !params.enable) {
            return [verifierAccount, quoteInit];
        }
        const [permissionAccount, permissionInit] = AttestationPermissionAccount.createInstruction(this.program, payer, {
    ...
    async createFunctionInstruction(payer, params, wallet, options) {
        const queueAuthority = params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const [functionAccount, functionInit] = await FunctionAccount.createInstruction(this.program, payer, { ...params, attestationQueue: this }, wallet, options);
        if (params.enable) {
    ...
    async addMrEnclaveInstruction(payer, params, options) {
    ...
    async addMrEnclave(params, options) {
        return await this.addMrEnclaveInstruction(this.program.walletPubkey, params, options).then((txn) => this.program.signAndSend(txn, options));
    }
    async removeMrEnclaveInstruction(payer, params, options) {
        const authority = params.authority?.publicKey ?? payer;
        const signers = params.authority ? [params.authority] : [];
    ...
    async removeMrEnclave(params, options) {
        return await this.removeMrEnclaveInstruction(this.program.walletPubkey, params, options).then((txn) => this.program.signAndSend(txn, options));
    }
    /**
    ...
    static async bootstrapNewQueue(program, params, options) {
        ...
        if (params?.verifierEnclave) {
            ...
            ixns.push(types.attestationQueueAddMrEnclave(program, {
                params: {
                    mrEnclave: Array.from(parseRawMrEnclave(params?.verifierEnclave ?? "", true)),
                },
            }, {
                queue: attestationQueueKeypair.publicKey,
                authority: authority.publicKey,
        ...
        const verifierQuotePermissions1 = AttestationPermissionAccount.fromSeed(program, authority.publicKey, attestationQueueKeypair.publicKey, verifierKeypair1.publicKey);
        ...
        ixns.push(types.verifierQuoteRotate(program, {
            params: {
                registryKey: Array.from(parseRawBuffer(params?.registryKey ?? "", 64)),
            },
        }, {
            verifier: verifierKeypair1.publicKey,
            authority: authority.publicKey,
        ...
        // quote #1 heartbeat
        ixns.push(types.verifierHeartbeat(program, { params: {} }, {
            verifier: verifierKeypair1.publicKey,
        ...
        const txns = TransactionObject.packIxns(program.walletPubkey, ixns, signers, options);
...
export { AttestationQueueAccount };
