...
class FunctionRequestAccount extends account_js_1.Account {
    ...
    static async createInstruction(program, payer, params, options) {
        const requestKeypair = params.keypair ?? web3_js_2.Keypair.generate();
        ...
        let instruction = types.functionRequestInit(program, {
            params: {
        ...
        instruction = new web3_js_1.TransactionInstruction({
            ...
            keys: instruction.keys.map((accountMeta) => accountMeta.pubkey.equals(program.attestationProgramId)
                ? {
        ...
        return [
            ...
            new TransactionObject_js_1.TransactionObject(payer, [instruction], params.functionAuthority
                ? [requestKeypair, params.functionAuthority]
                : [requestKeypair], options),
        ];
    }
    ...
    async getBalance() {
        ...
        if (balance === null) {
            throw new errors.AccountNotFoundError(`Function escrow`, this.getEscrow());
        }
        return balance;
    }
    async getBalanceBN() {
