 ...
 * Result: [u8;32]
 */
class SwitchboardWallet extends account_js_1.Account {
    static accountName = "SwitchboardWallet";
    _tokenWallet = undefined;
    /**
     * Returns the size of an on-chain {@linkcode SwitchboardWallet}.
     */
    size = this.program.attestationAccount.switchboardWallet.size;
    get tokenWallet() {
        if (!this._tokenWallet) {
            this._tokenWallet = this.program.mint.getAssociatedAddress(this.publicKey);
        }
    ...
    static parseName(name) {
        ...
        return (0, utils_js_1.parseRawBuffer)(nameSeed, 32);
    }
    static fromSeed(program, attestationQueue, authority, name) {
        const nameSeed = SwitchboardWallet.parseName(name);
        const walletPubkey = web3_js_1.PublicKey.findProgramAddressSync([
    ...
    async loadData() {
        ...
        if (!tokenAccountInfo) {
            throw new errors.AccountNotFoundError("SwitchboardWallet tokenWallet", this.tokenWallet);
        }
        (0, assert_1.default)(accountInfos.length === 0);
    ...
    static async createInstruction(program, payer, attestationQueue, authority, name, maxLen, options) {
        ...
        const walletInitIxn = types.walletInit(program, { params: { name: Buffer.from(nameSeed), maxLen: maxLen ?? null } }, {
        ...
        return [
            switchboardWallet,
            new TransactionObject_js_1.TransactionObject(payer, [walletInitIxn], [], options),
        ];
    }
    ...
    async fundInstruction(payer, params, options) {
        ...
        if (params.funderTokenWallet) {
            ...
            funderTokenWallet = SwitchboardProgram_js_1.SB_ATTESTATION_PID;
        }
        ...
        ixn.keys.push(...walletState.resources
            .filter((r) => !web3_js_1.PublicKey.default.equals(r))
            .map((r) => {
            return {
                pubkey: r,
                isSigner: false,
        ...
        ixns.push((0, utils_js_1.handleOptionalPubkeys)(ixn));
        return new TransactionObject_js_1.TransactionObject(payer, ixns, params.funderAuthority ? [params.funderAuthority] : [], options);
    }
    ...
    async wrapInstruction(payer, amount, options) {
        ...
        return new TransactionObject_js_1.TransactionObject(payer, [ixn], [], options);
    }
    ...
    async withdrawInstruction(payer, amount, destinationWallet, options) {
        const walletState = await this.loadData();
        const mint = await mint_js_1.Mint.load(this.program.provider, walletState.mint);
        ...
        return new TransactionObject_js_1.TransactionObject(payer, createTokenWalletIxn ? [createTokenWalletIxn, ixn] : [ixn], [], options);
    }
