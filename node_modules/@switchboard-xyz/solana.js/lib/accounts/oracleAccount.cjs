 ...
 */
class OracleAccount extends account_js_1.Account {
    static accountName = "OracleAccountData";
    static size = 636;
    /**
     * Get the size of an {@linkcode OracleAccount} on-chain.
     */
    size = this.program.account.oracleAccountData.size;
    /**
    ...
    async getPermissions(_oracle, _queueAccount, _queue) {
        ...
        const queueAccount = _queueAccount ?? new queueAccount_js_1.QueueAccount(this.program, oracle.queuePubkey);
    ...
    static async createInstructions(program, payer, params, options) {
        ...
        const oracleInit = new TransactionObject_js_1.TransactionObject(payer, [
        ...
        return [oracleAccount, TransactionObject_js_1.TransactionObject.pack(txns, options)];
    }
    ...
    async heartbeat(params, opts) {
        ...
        const queueAccount = params?.queueAccount ??
            new queueAccount_js_1.QueueAccount(this.program, oracle.queuePubkey);
        ...
        const heartbeatTxn = new TransactionObject_js_1.TransactionObject(this.program.walletPubkey, [
    ...
    async withdrawInstruction(payer, params, opts) {
        ...
        const oracle = await this.loadData();
        const queueAccount = new queueAccount_js_1.QueueAccount(this.program, oracle.queuePubkey);
        ...
        if (params.unwrap) {
            ...
            if (!withdrawDestinationAccountInfo?.owner.equals(web3_js_1.SystemProgram.programId)) {
            ...
            const txn = new TransactionObject_js_1.TransactionObject(payer, ixns, params.authority
        ...
        return new TransactionObject_js_1.TransactionObject(payer, [withdrawIxn], params.authority ? [params.authority] : [], opts);
    }
    ...
    getPermissionAccount(queuePubkey, queueAuthority, grantee = this.publicKey) {
        return permissionAccount_js_1.PermissionAccount.fromSeed(this.program, queueAuthority, queuePubkey, grantee);
    }
