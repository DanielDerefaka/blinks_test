...
const SolanaClock_js_1 = require("../SolanaClock.cjs");
...
const bufferRelayAccount_js_1 = require("./bufferRelayAccount.cjs");
...
const queueDataBuffer_js_1 = require("./queueDataBuffer.cjs");
const vrfAccount_js_1 = require("./vrfAccount.cjs");
 ...
 */
class QueueAccount extends account_js_1.Account {
     ...
     */
    size = this.program.account.oracleQueueAccountData.size;
    /**
     * Returns the queue's name buffer in a stringified format.
     */
    static getName = (queue) => (0, common_1.toUtf8)(queue.name);
    /**
     * Returns the queue's metadata buffer in a stringified format.
     */
    static getMetadata = (queue) => (0, common_1.toUtf8)(queue.metadata);
    ...
    async loadData() {
        ...
        this.dataBuffer = new queueDataBuffer_js_1.QueueDataBuffer(this.program, data.dataBuffer);
        return data;
    }
    /**
    ...
    static async createInstructions(program, payer, params, options) {
        ...
        queueAccount.dataBuffer = new queueDataBuffer_js_1.QueueDataBuffer(program, dataBuffer.publicKey);
        const queueSize = params.queueSize ?? 500;
        const queueDataSize = queueDataBuffer_js_1.QueueDataBuffer.getAccountSize(queueSize);
        ...
        const txn = new TransactionObject_js_1.TransactionObject(payer, [
            ...
            types.oracleQueueInit(program, {
                params: {
                    ...
                    consecutiveFeedFailureLimit: new common_1.BN(params.consecutiveFeedFailureLimit ?? 1000),
                    consecutiveOracleFailureLimit: new common_1.BN(params.consecutiveOracleFailureLimit ?? 1000),
                    queueSize: queueSize,
    ...
    static async create(program, params, options) {
        const [account, txnObject] = await this.createInstructions(program, program.walletPubkey, params, options);
    ...
    payer, params, options) {
        ...
        if (params.teeOracle &&
            (!params.authority || !(params.authority instanceof web3_js_1.Keypair))) {
            throw new Error(`Need to provide authority keypair when creating a teeOracle`);
        }
        const [oracleAccount, createOracleTxnObject] = await oracleAccount_js_1.OracleAccount.createInstructions(this.program, payer, {
            ...params,
            queueAccount: this,
        }, options);
        ...
        const [permissionAccount, createPermissionTxnObject] = permissionAccount_js_1.PermissionAccount.createInstruction(this.program, payer, {
        ...
        if (params.enable &&
            ...
            const permissionSetTxn = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_js_1.PermitOracleHeartbeat(),
                enable: true,
        ...
        return [
            oracleAccount,
            TransactionObject_js_1.TransactionObject.pack([...createOracleTxnObject, createPermissionTxnObject], options),
        ];
    }
    /**
     ...
     * ```
     */
    async createOracle(params, options) {
        const signers = [];
        ...
        const [oracleAccount, txn] = await this.createOracleInstructions(this.program.walletPubkey, params, options);
     ...
     * ```
     */
    async createFeedInstructions(payer, params, options) {
        const queueAuthorityPubkey = params.queueAuthority
            ? params.queueAuthority.publicKey
            : params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const pre = [];
        ...
        if (params.jobs && Array.isArray(params.jobs)) {
            for await (const job of params.jobs) {
                if ("data" in job) {
                    const [jobAccount, jobInit] = jobAccount_js_1.JobAccount.createInstructions(this.program, payer, {
                        data: job.data,
                        name: job.name ?? "",
                        authority: job.authority,
                        expiration: job.expiration,
                        variables: job.variables,
                        keypair: job.keypair ?? web3_js_1.Keypair.generate(),
                    }, options);
                    pre.push(...jobInit);
                    jobs.push({ job: jobAccount, weight: job.weight ?? 1 });
                }
                else if ("pubkey" in job) {
                    const jobAccount = new jobAccount_js_1.JobAccount(this.program, job.pubkey);
        ...
        const [aggregatorAccount, aggregatorInit] = await aggregatorAccount_js_1.AggregatorAccount.createInstruction(this.program, payer, {
            ...params,
            queueAccount: this,
        ...
        const [leaseAccount, leaseInit] = await leaseAccount_js_1.LeaseAccount.createInstructions(this.program, payer, {
        ...
        const [permissionAccount, permissionInit] = permissionAccount_js_1.PermissionAccount.createInstruction(this.program, payer, {
            granter: this.publicKey,
            authority: queueAuthorityPubkey,
        ...
        if (params.slidingWindow !== undefined && params.slidingWindow === true) {
            const setResolutionMode = aggregatorAccount.setResolutionModeInstruction(payer, {
                authority: undefined,
        ...
        // set priority fees
        if ((params.basePriorityFee !== undefined && params.basePriorityFee > 0) ||
        ...
        if (params.crankPubkey) {
            const [permissionAccount, permissionBump] = permissionAccount_js_1.PermissionAccount.fromSeed(this.program, queueAuthorityPubkey, this.publicKey, aggregatorAccount.publicKey);
            const leaseEscrow = this.program.mint.getAssociatedAddress(leaseAccount.publicKey);
            post.push(new TransactionObject_js_1.TransactionObject(payer, [
                types.crankPush(this.program, {
                    ...
                    dataBuffer: params.crankDataBuffer ??
                        (await new crankAccount_js_1.CrankAccount(this.program, params.crankPubkey).loadData()).dataBuffer,
                }),
            ], [], options));
        }
        if (params.historyLimit && params.historyLimit > 0) {
            const historyBufferInit = (await aggregatorHistoryBuffer_js_1.AggregatorHistoryBuffer.createInstructions(this.program, payer, {
                aggregatorAccount,
        ...
        if (params.authority && !params.authority.equals(payer)) {
            post.push(aggregatorAccount.setAuthorityInstruction(payer, {
                newAuthority: params.authority,
            }, options));
        }
        const packed = TransactionObject_js_1.TransactionObject.pack([
            ...(pre.length ? TransactionObject_js_1.TransactionObject.pack(pre) : []),
            ...(txns.length ? TransactionObject_js_1.TransactionObject.pack(txns) : []),
            ...(post.length ? TransactionObject_js_1.TransactionObject.pack(post) : []),
        ], options);
    ...
    async createFeed(params, options) {
        ...
        const [aggregatorAccount, txns] = await this.createFeedInstructions(this.program.walletPubkey, params, options);
     ...
     * ```
     */
    async createCrankInstructions(payer, params, options) {
        return await crankAccount_js_1.CrankAccount.createInstructions(this.program, payer, {
            ...params,
            queueAccount: this,
        }, options);
    }
     ...
     * ```
     */
    async createCrank(params, options) {
        const [crankAccount, txn] = await this.createCrankInstructions(this.program.walletPubkey, params, options);
     ...
     * ```
     */
    async createVrfInstructions(payer, params, options) {
        const queueAuthorityPubkey = params.queueAuthority
            ? params.queueAuthority.publicKey
            : params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const [vrfAccount, vrfInit] = await vrfAccount_js_1.VrfAccount.createInstructions(this.program, payer, {
        ...
        permissionAccount, permissionInit,] = permissionAccount_js_1.PermissionAccount.createInstruction(this.program, payer, {
            granter: this.publicKey,
        ...
        if (params.enable &&
            ...
            const permissionSet = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_js_1.PermitVrfRequests(),
                enable: true,
                queueAuthority: params.queueAuthority,
            }, options);
     ...
     * ```
     */
    async createVrf(params, options) {
        const [vrfAccount, txn] = await this.createVrfInstructions(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(txn, options);
    ...
    async createBufferRelayerInstructions(payer, params) {
        ...
        if ("data" in params.job) {
            const [jobAccount, jobInit] = jobAccount_js_1.JobAccount.createInstructions(this.program, payer, {
                ...
                keypair: params.job.keypair ?? web3_js_1.Keypair.generate(),
            });
            txns.push(...jobInit);
            job = jobAccount;
        }
        else if (params.job instanceof web3_js_1.PublicKey) {
            const jobAccount = new jobAccount_js_1.JobAccount(this.program, params.job);
            // should we verify its a valid job account?
            job = jobAccount;
        }
        else if (params.job instanceof jobAccount_js_1.JobAccount) {
            job = params.job;
        }
        else {
            throw new Error(`Failed to create BufferRelayer job account. 'data' or 'pubkey' was not defined in jobDefinition`);
        }
        const [bufferAccount, bufferInit] = await bufferRelayAccount_js_1.BufferRelayerAccount.createInstructions(this.program, payer, {
            name: params.name,
    ...
    async createVrfLiteInstructions(payer, params) {
        const queueAuthorityPubkey = params.queueAuthority
            ...
            : params.queueAuthorityPubkey ?? (await this.loadData()).authority;
        const txns = [];
        const [vrfLite, vrfLiteInit] = await vrfLiteAccount_js_1.VrfLiteAccount.createInstruction(this.program, payer, {
            ...params,
            queueAccount: this,
        });
        txns.push(vrfLiteInit);
        const [permissionAccount] = permissionAccount_js_1.PermissionAccount.fromSeed(this.program, queueAuthorityPubkey, this.publicKey, vrfLite.publicKey);
        if (params.enable &&
            (params.queueAuthority || queueAuthorityPubkey.equals(payer))) {
            const permissionSet = permissionAccount.setInstruction(payer, {
                permission: new SwitchboardPermission_js_1.PermitVrfRequests(),
    ...
    async loadOracles() {
        ...
        if (this.dataBuffer) {
            ...
            queue = new queueDataBuffer_js_1.QueueDataBuffer(this.program, queueData.dataBuffer);
        }
    ...
    async loadOracleAccounts(_oracles) {
        ...
        return await oracleAccount_js_1.OracleAccount.fetchMultiple(this.program, oraclePubkeys);
    }
    async loadActiveOracleAccounts(_queue) {
        const queue = _queue ?? (await this.loadData());
        const oracles = await this.loadOracleAccounts();
        const unixTimestamp = (await SolanaClock_js_1.SolanaClock.fetch(this.program.connection))
            .unixTimestamp;
            ...
            types.oracleQueueSetConfig(this.program, {
                params: {
                    ...
                    consecutiveFeedFailureLimit: params.consecutiveFeedFailureLimit
                        ? new common_1.BN(params.consecutiveFeedFailureLimit)
                        : null,
                    consecutiveOracleFailureLimit: params.consecutiveOracleFailureLimit
                        ? new common_1.BN(params.consecutiveOracleFailureLimit)
                        : null,
