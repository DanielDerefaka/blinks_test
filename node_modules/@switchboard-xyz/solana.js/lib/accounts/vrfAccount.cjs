 ...
 * Result: [u8;32]
 */
class VrfAccount extends account_js_1.Account {
    static accountName = "VrfAccountData";
    /**
     * Returns the size of an on-chain {@linkcode VrfAccount}.
     */
    size = this.program.account.vrfAccountData.size;
    /**
    ...
    static async createInstructions(program, payer, params, options) {
        ...
        return [
            vrfAccount,
            new TransactionObject_js_1.TransactionObject(payer, ixns, [params.vrfKeypair], options),
        ];
    }
    /**
    ...
    static async create(program, params, options) {
        const [vrfAccount, vrfInitTxn] = await VrfAccount.createInstructions(program, program.walletPubkey, params, options);
    ...
    async requestRandomnessInstruction(payer, params, options) {
        ...
        const queueAccount = params.queueAccount ?? new queueAccount_js_1.QueueAccount(this.program, vrf.oracleQueue);
        ...
        const requestRandomness = new TransactionObject_js_1.TransactionObject(payer, [
    ...
    async requestRandomness(params, options) {
        ...
        const txnSignature = await this.program.signAndSend(requestRandomness, options);
        return txnSignature;
    }
    proveAndVerifyInstructions(params, options, numTxns = 40) {
        ...
        const txns = Array.from(Array(numTxns).keys()).map((i) => {
            ...
            return new TransactionObject_js_1.TransactionObject(this.program.walletPubkey, [proveIxn], [], {
    ...
    async proveAndVerify(params, options, numTxns = 40) {
        ...
        if (!oracleTokenWallet || !oracleAuthority) {
            const oracleAccount = new oracleAccount_js_1.OracleAccount(this.program, oraclePubkey);
    ...
    setCallbackInstruction(payer, params, options) {
        ...
        ], params.authority instanceof web3_js_1.Keypair ? [params.authority] : [], options);
    }
    async setCallback(params, options) {
        const setCallbackTxn = this.setCallbackInstruction(this.program.walletPubkey, params, options);
        const txnSignature = await this.program.signAndSend(setCallbackTxn, options);
        return txnSignature;
    }
    ...
    async fetchAccounts(_vrf, _queueAccount, _queue) {
        ...
        const queueAccount = _queueAccount ?? new queueAccount_js_1.QueueAccount(this.program, vrf.oracleQueue);
    ...
    async nextResult(roundId, timeout = 30000) {
        ...
        if (roundId) {
            ...
            if (vrf.status.kind === "StatusVerifying") {
                ...
                id = vrf.counter.add(new common_1.BN(1));
            }
        }
        let ws;
        const closeWebsocket = async () => {
    ...
    async closeAccountInstruction(payer, params) {
        ...
        const queueAccount = params?.queueAccount ??
            new queueAccount_js_1.QueueAccount(this.program, vrfLite.oracleQueue);
        ...
        const vrfClose = new TransactionObject_js_1.TransactionObject(payer, [
            (0, index_js_1.vrfCloseAction)(this.program, {
                params: {
                    stateBump: this.program.programState.bump,
